<?php 
    ...
    if(!empty($_POST))
    {
        ...
        if(empty($_POST['nombre']) || empty($_POST['descripcion']) || empty($_POST['cantidad']) || empty($_POST['codigo'])){
            ...
            $sql_update = mysqli_query($conexion, "UPDATE materiales SET nombre = '$nombre', descripcion = '$descripcion', cantidad = '$cantidad', codigo = ' $codigo' WHERE idmaterial = $idmaterial");
    ...
    $sql = mysqli_query($conexion, "SELECT * FROM materiales WHERE idmaterial = $idmaterial and estado = 1");
